...
async function clearFirestoreCollection(collectionPath: string) {
  ...
  for (let i = 0; i < docs.length; i += batchSize) {
    ...
    await batch.commit();
    console.log(
      `Deleted ${Math.min(i + batchSize, docs.length)} / ${docs.length}`,
    );
  }
}

async function clearRealtimeBookings(path: string) {
  try {
